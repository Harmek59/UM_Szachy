...
const float AROUND_KING_VALUE = 0.05;
...
float evaluate(thc::ChessRules &board) {
    ...
    thc::MOVELIST moveList = thc::MOVELIST();
    board.GenLegalMoveList(&moveList);
    for (auto &move: moveList.moves) {
        val += ifAroundKing(board.bking_square, move.dst, true);
        val -= ifAroundKing(board.wking_square, move.dst, false);
    }
    return val;
}
